...
`convex/firewallCrawl.ts` reads **`CONVEX_SITE_URL`** (set automatically on the Convex deployment for `*.convex.site`). Local/docs may also mention `NEXT_PUBLIC_CONVEX_SITE_URL` or `VITE_PUBLIC_SITE_URL` for the public fortress URL — never swap those into `VITE_CONVEX_URL`.
