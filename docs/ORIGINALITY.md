# CeilingGate originality (All Gas)

## Must
Email claim + public receipt URL → Firecrawl scrape → ResidualGates line ledger (claimed ≤ interior) → GRANT/REFUSE + plain English + mask chip.

## Must not
- waynesutton AgentMail chat-demo rebrand
- Firecrawl docs-search chat
- Generic AI inbox yes/no (Attest-like)
- Magpie / Square / T&E commercial pitch
- Fake superiority metrics

## Demo signature
`te-grant` then `te-refuse` ($1 lodging + misc) via **Demo signature: GRANT → $1 REFUSE**.
